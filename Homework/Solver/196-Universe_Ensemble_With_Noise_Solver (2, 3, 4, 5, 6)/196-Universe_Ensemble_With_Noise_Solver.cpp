...
unsigned long long visit_token = 0; // ป้องกันระเบิดเวลา Overflow
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (ner >= 0 && ner < N && nec >= 0 && nec < N) {
                ...
                if (global_visited[n_state_id] != visit_token) {
                    ...
                    // 🔥 อัปเกรด 2: Omnidirectional Hovering (ดักทุกทิศทาง)
                    if (goal_c > ntc && nec < ntc) penalty += 3; // จะไปขวา แต่อยู่ซ้าย
                    if (goal_c < ntc && nec > ntc) penalty += 3; // จะไปซ้าย แต่อยู่ขวา
                    if (goal_r > ntr && ner < ntr) penalty += 3; // จะลงล่าง แต่อยู่บน
                    if (goal_r < ntr && ner > ntr) penalty += 3; // จะขึ้นบน แต่อยู่ล่าง
        ...
        for (int r = 0; r < N; ++r) {
            for (int c = 0; c < N; ++c) {
                if (grid[get_idx(r, c)] == goal_val && !locked[get_idx(r, c)]) {
                    ...
                    // 🔥 อัปเกรด 1: Crowding Penalty (รังเกียจกำแพง) 
                    int crowded_penalty = 0;
                    int ddr[] = {1, -1, 0, 0}, ddc[] = {0, 0, 1, -1};
                    for(int d=0; d<4; ++d) {
                        int nr = r + ddr[d], nc = c + ddc[d];
                        if(nr >= 0 && nr < N && nc >= 0 && nc < N) {
                            if(locked[get_idx(nr, nc)]) crowded_penalty += 25; 
                        } else crowded_penalty += 25; 
                    }
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        backup_filename = "Chaos_Auto_" + to_string(time(NULL)) + "_" + to_string(rand() % 1000) + ".txt";
    }
    ...
    // (สร้างเป้าหมายเหมือนเดิมเป๊ะ ผมขอละโค้ดส่วนนี้ไว้ให้สั้นลง แต่มีครบในไฟล์ที่คุณรันครับ)
    ...
    cerr << "[INFO] Running 196-Universe Micro-Optimized Engine...\n\n";
    ...
    while (true) {
        ...
        try {
            ...
            if (mutated_order.size() > 4) {
                // 🔥 อัปเกรด 3: เพิ่มท่ามั่วแบบ Block Swap 
                int mut_type = rand() % 4; 
                if (mut_type == 0) {
                    ...
                    int block_size = (rand() % 3) + 2; // สลับยกแก๊ง 2-4 แผ่น
            ...
            if (iterations % 50 == 0) {
                ...
                int h = elapsed_seconds / 3600, m = (elapsed_seconds % 3600) / 60, s = elapsed_seconds % 60;
            ...
            if (path != "FAILED" && path.length() < min_len) {
                ...
                cerr << "\n\n🔥 NEW MICRO-OPT RECORD! Moves: " << min_len << " (" << best_name << ")\n";
