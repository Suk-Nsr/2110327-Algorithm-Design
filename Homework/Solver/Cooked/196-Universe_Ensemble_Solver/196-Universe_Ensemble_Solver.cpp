...
int main() {
    ...
    // ========================================================
    // 5. Inside-Out Concentric Spiral (4 ทิศทาง - จบที่มุมเสมอเพื่อกันทางตัน!)
    // ========================================================
    vector<vector<pair<int, int>>> layers;
    int top = 1, bottom = M, left = 1, right = M;
    
    // สร้างวงแหวนจากข้างนอกเข้าข้างใน
    while (top <= bottom && left <= right) {
        vector<pair<int, int>> layer;
        if (top == bottom && left == right) { // ตรงกลางสุด (กรณี N เป็นเลขคู่แล้วเหลือแกนกลาง)
    ...
    // พลิกจากศูนย์กลางแผ่ออกข้างนอก (Inside-Out)
    ...
    for (const auto& l : layers) {
        ...
        // หาขอบเขตของวงแหวนรอบนี้เพื่อระบุพิกัดมุมทั้ง 4
        ...
        // หา Index ของมุมทั้ง 4 ในวงแหวนนี้
        ...
        // ฟังก์ชันหมุนวงแหวนให้ "จบที่พิกัดมุม" (ตัวถัดไปจากมุมจะกลายเป็นตัวเริ่มแรกสุด)
        auto add_rotated = [](vector<pair<int,int>>& target, const vector<pair<int,int>>& src, int end_idx) {
            int start_idx = (end_idx + 1) % src.size(); // เริ่มแผ่นถัดจากมุม
        ...
        // ยัดลงใน 4 รูปแบบ
        add_rotated(sp_tl, l, idx_tl); // บังคับจบที่มุมซ้ายบน
        add_rotated(sp_tr, l, idx_tr); // บังคับจบที่มุมขวาบน
        add_rotated(sp_br, l, idx_br); // บังคับจบที่มุมขวาล่าง
        add_rotated(sp_bl, l, idx_bl); // บังคับจบที่มุมซ้ายล่าง
    }
    
    strategies.push_back({"Inside-Out Spiral (End Top-Left Corner)", sp_tl});
    strategies.push_back({"Inside-Out Spiral (End Top-Right Corner)", sp_tr});
    strategies.push_back({"Inside-Out Spiral (End Bottom-Right Corner)", sp_br});
    strategies.push_back({"Inside-Out Spiral (End Bottom-Left Corner)", sp_bl});
    
    // 🔥 7 Personalities (Weights) 🔥
    vector<pair<int, int>> weights = {
        {1, 5},   // Balanced (คลาสสิก)
        {3, 5},   // Lazy Empty (ประหยัดก้าวเดินรถเปล่า)
        {1, 8},   // Greedy Goal (คว้ากระเบื้องใกล้เป้าก่อน)
        {1, 10},  // Hyper Greedy Goal (โฟกัสเป้าหมายแบบสุดโต่ง)
        {1, 2},   // Slightly Goal Oriented (สมดุลค่อนไปทางเป้า)
        {5, 2},   // Super Lazy Empty (เครื่องดูดฝุ่น ช่องว่างขี้เกียจเดินสุดๆ)
        {1, 1}    // Pure Shortest (อิงระยะทางกระจัดรวมเพียวๆ)
    };

    cerr << "[INFO] Running Ultimate 196-Universe Multi-Heuristic Ensemble...\n\n";
